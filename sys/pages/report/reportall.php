...
ORDER BY pr.p_type";
	...
	$sqlCountAllToday = "SELECT SUM(ord_qty) qty, SUM(ord_amount) amount FROM tb_order_detail WHERE ord_dates = '$selectDate'";
	$result_sqlCountAllToday  = mysql_query($sqlCountAllToday);
	$row_sqlCountAllToday = mysql_fetch_array($result_sqlCountAllToday);
	
	$sqlAmountToday = "SELECT SUM(ord_amount) amount FROM tb_order_detail WHERE ord_dates = '$selectDate'";
