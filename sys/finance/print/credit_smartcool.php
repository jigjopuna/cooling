...
<?php 
		...
		$result = mysql_query("SELECT p.po_id, p.po_name, s.sl_name, s.sl_credit_date, p.po_price,  p.po_mudjum,p.po_price-p.po_mudjum remains,  p.po_bill_img, p.po_comment, p.po_date, p.po_credit_due_date, DATEDIFF( NOW(), p.po_credit_due_date) as countday
								...
								WHERE p.po_shop = 20 AND p.po_credit = 1 
								ORDER BY p.po_id DESC");
		$num = mysql_num_rows($result);
		
		$rowcount = mysql_fetch_array(mysql_query("SELECT s.sl_name, SUM(p.po_price) yodtem, SUM(p.po_mudjum) mudjum, SUM(p.po_price-p.po_mudjum) remains FROM tb_po p JOIN tb_sellers s ON s.sl_id = p.po_shop WHERE p.po_shop = 20 AND p.po_credit = 1"));		

        $sqlcopr = mysql_fetch_array(mysql_query("SELECT sl_corp FROM tb_sellers WHERE sl_id = 20"));
