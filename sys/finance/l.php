...
<?php 
	...
	$sql_cash = "SELECT e.e_id, e.e_name, SUM(po_price) poprice1 FROM tb_po p JOIN tb_emp e ON p.po_buyer = e.e_id WHERE p.po_date = '$today' AND p.po_credit != 1 GROUP BY po_buyer";
	$result_cash = mysql_query($sql_cash );
	$num_cash = mysql_num_rows($result_cash);
	
	// ซื้อของแบบเครดิต ยังไม่จ่ายเงิน
	$sql_credit = "SELECT e.e_id, e.e_name, SUM(po_price) poprice1 FROM tb_po p JOIN tb_emp e ON p.po_buyer = e.e_id WHERE p.po_date = '$today' AND p.po_credit = 1 GROUP BY po_buyer";
	$result_credit = mysql_query($sql_credit);
	$num_credit = mysql_num_rows($result_credit);
	
	//ยอดเงินเข้าทั้งหมดวันนี้รวมของทุกคน
	$in_come = mysql_fetch_array(mysql_query("SELECT SUM(pay_amount) income FROM tb_ord_pay WHERE pay_date = '$today' "));
	$incomes = number_format($in_come['income'], 0, '.', ',');
	
	
	//เงินเข้าวันนี้แยกตามคนรับเงิน
	$sql_income = "SELECT e.e_id, e.e_name, SUM(pay_amount) income FROM tb_ord_pay ord JOIN tb_emp e ON e.e_id = ord.o_emp_receive WHERE pay_date = '$today' GROUP BY ord.o_emp_receive";
	...
	$sumtoday = mysql_fetch_array(mysql_query("SELECT SUM(po_price) paytoday FROM tb_po WHERE po_date = '$today'"));
	$paytoday = number_format($sumtoday['paytoday'], 0, '.', ',');
	...
	$msg = "\nสรุปประวันที่ : ".$today."\nเงินเข้าแยกตามคน \n".$income1."\n\nรวมเงินเข้าทั้งหมด ". $incomes ." บาท \n". "ค่าใช้จ่ายเงินสด \n". $paycash. " \n\nเครดิต : " .$paycredit. " \nคงค้างเครดิต : " .$remain1. "\n\nรวมค่าใช้จ่ายทั้งหมด ".$paytoday. "บาท"."\nเงินกองกลางเหลือ ".$cur_cash." บาท";
	//echo $msg;
	//exit();
