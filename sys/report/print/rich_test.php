...
<?php 
		$dates = date("Y-m-d");
		$sql_debt = "SELECT  s.sl_name, s.sl_id, s.sl_type, SUM(p.po_price) sumprice, SUM(p.po_mudjum) mudjum, SUM(p.po_price)-SUM(p.po_mudjum) AS yod ,  COUNT(p.po_id) AS bill , s.sl_corp
											  FROM tb_po p JOIN tb_sellers s ON p.po_shop = s.sl_id
											  WHERE s.sl_publish = 1 AND p.po_credit = 1 AND p.po_credit_complete = 0
											  GROUP BY p.po_shop
			...
			for($i=1; $i<=$num; $i++){
					...
					for($j=0; $j<=$num_debt; $j++){
					$row_debt = mysql_fetch_array($result_debt);
						$arr1[$i][$j] = $row_debt[$j+1];	
