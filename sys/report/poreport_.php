...
<?php 
	...
	if($rep_time==1){
		...
		$sql_eachshop = "SELECT s.sl_id, s.sl_name, a.raka
		 ...
		 //ค่าใช้จ่ายจริงๆ หมายถึงเงินที่ออกจากบัญชีจริงๆ ไม่ใช่เงินเครดิต แต่ยอดเต็มเครก็ลงระบบไว้แล้ว 
		    //ซื้อเครดิตแบมีมัดจำ
			$result_realpay = mysql_fetch_array(mysql_query("SELECT SUM(po_mudjum) realpay FROM tb_po WHERE po_credit = 1 AND po_credit_complete = 0 AND po_mudjum != 0 AND po_date LIKE '$select_month'"));
			
			//ซื้อขายเงินสด
			$result_sod = mysql_fetch_array(mysql_query("SELECT SUM(po_price) sod FROM tb_po WHERE po_credit = 0 AND po_date LIKE '$select_month'"));
			
		//ยอดเครดิตเดือนนี้
		$result_yodcredit = mysql_fetch_array(mysql_query("SELECT SUM(po_price) yodcredits FROM tb_po WHERE po_credit = 1 AND po_date LIKE '$select_month'"));
			
 		
			
			$row_realpay = $result_realpay['realpay'];
			$row_sod = $result_sod['sod'];
		    $yodcredit = $result_yodcredit['yodcredits'];
			
			
		//ดูว่าเราจ่ายเครดิตเก่าๆ ไปเท่าไรแล้วในเดือนนี้ รวมจ่าย remain credit pay
		$result_remre = mysql_fetch_array(mysql_query("SELECT SUM(c.cash_out) cashout FROM tb_po p JOIN tb_cash_center c ON p.po_id = c.cash_po WHERE p.po_credit = 1 AND p.po_credit_complete = 1 AND c.cash_times LIKE '$select_month'"));
		$cashout_credit = $result_remre['cashout'];
		
		//ดูว่าเราจ่ายเครดิตเก่าๆ ไปเท่าไรแล้วในเดือนนี้ รายละเอียด 
		$sql_remre =   "SELECT p.po_name, p.po_shop, p.po_price, p.po_mudjum, c.cash_out, c.cash_times
						FROM tb_po p JOIN tb_cash_center c ON p.po_id = c.cash_po WHERE p.po_credit = 1 AND p.po_credit_complete = 1 AND c.cash_times LIKE '2019-12%' 
						ORDER BY c.cash_times DESC";
		$result_remlist =  mysql_query($sql_remre);
		$num_remlist = mysql_num_rows($result_remlist);
        ...
        <div id="page-wrapper">
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
								สรุปรายการจ่ายซื้อของ  <?php echo $times;?><br>
								ซื้อสด <?php echo number_format($row_sod, 2, '.', ','); ?> บาท <br>
								ยอดเครดิตเดือนนี้  <?php echo number_format($yodcredit, 2, '.', ','); ?> บาท<br>
								มัดจำ <?php echo number_format($row_realpay, 2, '.', ','); ?> บาท<br>
								เงินออกจากบัญชีจริงๆ  <?php echo number_format($row_sod+row_realpay, 2, '.', ','); ?> <br><br>
								
								ค่าใช้จ่ายทั้งหมดรวมเครดิตด้วย <?php echo number_format($rowsum, 2, '.', ','); ?>
                        </div>
                        <!-- /.panel-heading -->
			...
			<div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
								ยอดชำระเครดิต <?php echo number_format($cashout_credit, 2, '.', ','); ?>
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover data_table">
                                ...
                                <tbody>
                                    <?php 
										for($i=1; $i<=$num_remlist; $i++){
										  $row_remlist = mysql_fetch_array($result_remlist);
									  ?>
										<tr class="gradeA"> 
											<td><?php echo $i; ?></td>
											<td><?php echo $row_remlist['po_name']; ?></td>
											<td><?php echo $row_remlist['po_shop']; ?></td>
											<td><?php echo number_format($row_remlist['po_price'], 2, '.', ','); ?></td>
											<td><?php echo number_format($row_remlist['cash_out'], 2, '.', ','); ?></td>
											<td><?php echo $row_remlist['cash_times']; ?></td>
