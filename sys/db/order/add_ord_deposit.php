...
<?php 
	...
	if($result1) {
		...
		if($result1){
			$msg = ' ลูกค้า '.$custnames." ". " ฝาก อาหาร ".$prod.' จำวน '.number_format($ord_qty, 0, '.', ',').' kg ';	
			$res = notify_message($msg);			
			exit("<script>alert('บันทึกฝากสินค้าใหม่เรียบร้อยแล้วจร้า ^^ '); window.location='../../order/ord_deposit.php';</script>");
		}else{
			 exit("<script>alert('บันทึกฝากสินค้าไม่สำเร็จ ติดต่อผู้ดูแลระบบ1'); window.location='../../order/ord_deposit.php';</script>");
		}
	} else {
		 exit("<script>alert('บันทึกฝากสินค้าไม่สำเร็จ ติดต่อผู้ดูแลระบบ'); window.location='../../order/ord_deposit.php';</script>");
	}
?>
</body>
</html>     
