...
<?php 
   ...
   require_once('../include/thaibaht.php');
	...
	$nowall  = trim($_POST['nowall']);
	$r_height  = trim($_POST['r_height']);
	
	$chkthreeside  = trim($_POST['chkthreeside']);	 
	...
	echo "r_width2 = ".$r_width2."<br>";
	echo "r_length2 = ".$r_length2."<br>";
	echo "r_height = ".$r_height."<br>";
	echo "nowall = ".$nowall."<br>";
		...
		if($getfromcondensing > $get_chkMaxKwcw ){ //compare cooler max 
		  //  echo "คอนเด็นซิงมากกว่าจริงๆ นะ", '<br>';
			...
			if($temparature==1){
				...
				$sql_cooler = "SELECT * FROM tb_product p JOIN tb_category c ON c.cat_id = p.p_cate WHERE p.p_cw5 > '$total_result_t'-0.2 AND p.p_cate = 2 ORDER BY p.p_price_sell ASC Limit 0,1";
			}else{			
				 //echo "cw20","<br>";
				 $sql_cooler = "SELECT * FROM tb_product p JOIN tb_category c ON c.cat_id = p.p_cate WHERE p.p_cw20 > '$total_result_t'-0.2 AND p.p_cate = 2 ORDER BY p.p_price_sell ASC Limit 0,1";				
			}
...
<form method="post" action="pfq.php"id="form1">
    <div class="page">
		...
		<?
			//หาพื้นที่ห้อง (ผนัง/เพดาน) ไม่รวมพื้น
			  //เช็คก่อนว่าด้านไหนที่ไม่เอา
			if($nowall==1){
				// เอาทุกด้าน
				$area_room = ((($r_width*$r_height)*2)+ (($r_length*$r_height)*2) + ($r_length*$r_width)); 
				echo $area_room.' ตร.ม. เอาทุกด้าน <br>';
				$r_length2 = $r_length;
				$r_width2 = $r_width;
			}
			...
			if($nowall==2){
				...
				$r_width2 = 0;
				 // 1 =ไม่เอาด้านกว้าง
				$area_room = (($r_width*$r_height)+ (($r_length*$r_height)*2) + ($r_length*$r_width)); 
				echo $area_room.' ตร.ม. ไม่เอาด้านกว้าง <br>';
			}
			
			if($nowall==3){
				$r_length2 = 0;
				$r_width2 = $r_width;
				echo $area_room.' ตร.ม. ไม่เอาด้านยาว <br>';
				// 1 =ไม่เอาด้านยาว
				$area_room = ((($r_width*$r_height)*2)+ ($r_length*$r_height) + ($r_length*$r_width));

			}
			

			$sumside = $r_length + $r_length2 + $r_width + $r_width2;
			...
			<div id="product_price" style="margin-top:105px; clear:both">
				<table style="width: 100%; border: solid black 1px;  border-collapse: collapse;">
					...
					<tr>
						...
						<td colspan="2" class="l" align="center"><input name="r1q" style="width:45px;" class="punit" type="text" value="<?php echo $area_room?>"> ตร.ม.</td>
						<td class="l" align="right"><input name="r1p" class="punit" type="text" value="<?php echo number_format($row_isowall['pr_sell_price'], 2, '.', ',');?>"></td>
						<td class="l" align="right"><?php echo number_format($row_isowall['pr_sell_price']*$area_room, 2, '.', ','); ?></td>
					</tr>
